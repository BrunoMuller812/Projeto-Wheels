...
function AvailableBikes() {
    ...
    const { customerIdFromAdmin, customerNameFromAdmin } = location.state || {};
    const isCalledByAdmin = !!customerIdFromAdmin; 
    const effectiveCustomerId = customerIdFromAdmin || user?.customerId;
    const effectiveCustomerName = customerNameFromAdmin || user?.username || 'Cliente Desconhecido';
    ...
    const fetchBikes = async () => {
        ...
        setError(null);
        try {
            console.log("AvailableBikes: Buscando bicicletas da API...");
            ...
            console.error("AvailableBikes: Falha ao buscar bicicletas:", err);
    ...
    useEffect(() => {
        fetchBikes();
        console.log("AvailableBikes: Componente montado ou user/location.state atualizado.");
        console.log("AvailableBikes: user:", user);
        console.log("AvailableBikes: location.state:", location.state);
        console.log("AvailableBikes: customerIdFromAdmin:", customerIdFromAdmin);
        console.log("AvailableBikes: customerNameFromAdmin:", customerNameFromAdmin);
        console.log("AvailableBikes: effectiveCustomerId:", effectiveCustomerId);
        console.log("AvailableBikes: effectiveCustomerName:", effectiveCustomerName);
        console.log("AvailableBikes: isCalledByAdmin:", isCalledByAdmin);
    }, [user, location.state]); 
    ...
    const openRentalModal = (bike) => {
        ...
        if (!effectiveCustomerId) {
            setModalError(isCalledByAdmin
                ? 'Erro: Admin precisa selecionar/cadastrar um cliente antes de alugar uma bike.'
                : 'Erro: Você precisa estar logado com um cliente válido para alugar. Tente relogar.'
            );
            return; 
        }
    ...
    const handleProceedToPayment = (e) => {
        ...
        if (!effectiveCustomerId) {
            setModalError('Erro: Não foi possível obter o ID do cliente para o aluguel. Por favor, faça login ou selecione/cadastre um cliente (Admin).');
            return;
        }
        ...
        navigate('/payment', {
            state: {
                ...
                customerIdToUse: effectiveCustomerId, 
    ...
    return (
        <div style={pageStyles.container}>
            ...
            <p style={pageStyles.description}>
                Aqui você encontrará todas as bicicletas em nosso estoque prontas para aluguel.
                {isCalledByAdmin && customerNameFromAdmin && (
                    <span style={pageStyles.adminContextMessage}> (Alugando para: <strong>{customerNameFromAdmin}</strong>)</span>
                )}
            </p>
            ...
            {showRentalModal && selectedBikeForRental && (
                <div style={modalStyles.overlay}>
                    <div style={modalStyles.modal}>
                        <h3 style={modalStyles.modalTitle}>Alugar: {selectedBikeForRental.modelo}</h3>
                        {isCalledByAdmin && customerNameFromAdmin && (
                            <p style={modalStyles.adminContextMessage}>
                                <strong style={{color: '#007bff'}}>Alugando para: {customerNameFromAdmin}</strong>
                            </p>
                        )}
                        <form onSubmit={handleProceedToPayment} style={modalStyles.form}>
                            ...
                            <div style={modalStyles.buttonGroup}>
                                <button 
                                    ...
                                    disabled={!!modalError || !effectiveCustomerId} 
                                >
